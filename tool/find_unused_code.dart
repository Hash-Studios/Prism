...
const List<String> _generatedSuffixes = <String>['.g.dart', '.freezed.dart', '.gr.dart'];
...
final RegExp _importRe = RegExp(r"""^import\s+['"]([^'"]+)['"]""", multiLine: true);

/// Matches `export '...'` or `export "..."`.
final RegExp _exportRe = RegExp(r"""^export\s+['"]([^'"]+)['"]""", multiLine: true);
...
void main(List<String> args) {
  ...
  if (!File('$projectRoot/pubspec.yaml').existsSync()) {
    stderr.writeln('Error: pubspec.yaml not found. Run this script from the project root.');
    exitCode = 1;
    return;
  }
  ...
  final Map<String, List<String>> unusedSymbols = _findUnusedPublicSymbols(reachable, allFiles);
  ...
  if (jsonMode) {
    _outputJson(
      ...
      unusedSymbols: unusedSymbols.map((k, v) => MapEntry(_rel(k, projectRoot), v)),
    );
  } else {
    _outputTerminal(
      sourceCount: sourceCount,
...
List<String> _findUnreachable(Set<String> allFiles, Set<String> reachable) =>
    allFiles.where((String f) => !reachable.contains(f) && !_isGenerated(f)).toList();
...
bool _isGenerated(String path) {
  ...
  try {
    ...
    if (header.contains('GENERATED CODE') || header.contains('generated by')) return true;
  } catch (_) {}
  return false;
}
...
Map<String, List<String>> _findUnusedPublicSymbols(
    Set<String> reachable, Set<String> allFiles) {
...
void _outputTerminal({
  ...
  stdout.writeln('Analysed $sourceCount source files ($generatedCount generated excluded).');
  ...
  if (unreachable.isEmpty) {
    ...
    stdout.writeln('Files never reached from main.dart via import/export/part chains.');
    stdout.writeln();
  ...
  final int symbolCount = unusedSymbols.values.fold(0, (int s, List<String> l) => s + l.length);
  final String sTitle = '── UNUSED PUBLIC SYMBOLS ($symbolCount) ';
  stdout.writeln('\n$sTitle${'─' * (76 - sTitle.length)}');
  if (unusedSymbols.isEmpty) {
    stdout.writeln('  (none detected)');
  } else {
    stdout.writeln('Declared in reachable files, but never referenced elsewhere.');
    stdout.writeln('Note: DI/router-registered symbols may be false positives.');
    stdout.writeln();
...
void _generateHtml({
  ...
  final int unusedSymbolCount =
      unusedSymbols.values.fold(0, (int s, List<String> l) => s + l.length);
  ...
  final List<_FileInfo> unreachableInfo = unreachable.map((String f) {
    ...
    ..sort((_FileInfo a, _FileInfo b) => b.lines.compareTo(a.lines));

  // Group unreachable by top lib folder.
  final Map<String, List<_FileInfo>> unreachableByFolder = <String, List<_FileInfo>>{};
  ...
  final List<MapEntry<String, List<String>>> sortedSymbols = unusedSymbols.entries
      .map((MapEntry<String, List<String>> e) =>
          MapEntry<String, List<String>>(_rel(e.key, projectRoot), e.value))
      .toList()
    ..sort((MapEntry<String, List<String>> a, MapEntry<String, List<String>> b) =>
  ...
  html.writeln(_statCard('Unreachable Files', '${unreachable.length}', warn: unreachable.isNotEmpty));
  html.writeln(_statCard('Unused Symbols', '$unusedSymbolCount', warn: unusedSymbolCount > 0));
  html.writeln('</div>');

  // Known limitations notice
  html.writeln('<details class="notice"><summary>Known limitations &amp; false-positive sources</summary>');
  html.writeln('<ul>');
  html.writeln('<li><strong>Unused public symbols</strong> may be false positives for <code>@injectable</code> classes, auto_route-registered screens, and Hive adapter type IDs.</li>');
  html.writeln('<li>Conditional imports (<code>if (dart.library.html)</code>) — only the primary URI path is followed.</li>');
  html.writeln('<li>Dynamic/reflective references (<code>Type.toString()</code>) are not detectable.</li>');
  html.writeln('<li>Symbols used only in <code>test/</code> appear unused if that directory is excluded from the scan.</li>');
  html.writeln('</ul>');
  ...
  if (unreachable.isEmpty) {
    html.writeln('<p class="empty">All files are reachable from <code>lib/main.dart</code>.</p>');
  } else {
    html.writeln('<p class="hint">Files never reached from <code>lib/main.dart</code> via import / export / part chains. Sorted by line count (largest first).</p>');
    for (final MapEntry<String, List<_FileInfo>> folderEntry in unreachableByFolder.entries) {
      html.writeln('<details open>');
      html.writeln('<summary class="folder">${folderEntry.key} <span class="badge">${folderEntry.value.length}</span></summary>');
      html.writeln('<table>');
      html.writeln('<thead><tr><th>File</th><th class="num">Lines</th></tr></thead>');
      html.writeln('<tbody>');
      for (final _FileInfo info in folderEntry.value) {
        html.writeln('<tr><td class="path"><code>${info.path}</code></td><td class="num">${info.lines}</td></tr>');
      }
  ...
  if (sortedSymbols.isEmpty) {
    ...
    html.writeln('<p class="hint">Declared in reachable files, but never referenced in any other file. DI / router-registered symbols may be false positives.</p>');
