...
class UserProfileSetupGrid extends StatefulWidget {
  final String? email;
  const UserProfileSetupGrid({this.email, super.key});
...
class _UserProfileSetupGridState extends State<UserProfileSetupGrid> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadMoreSetups() async {
    if (seeMoreLoader) {
      return;
    }
    setState(() {
    ...
    try {
      await context.publicProfileAdapter(listen: false).seeMoreUserProfileSetups(widget.email);
    } finally {
      if (mounted) {
        setState(() {
          seeMoreLoader = false;
