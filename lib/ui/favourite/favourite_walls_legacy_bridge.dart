...
class LegacyFavouriteWallSnapshot {
  const LegacyFavouriteWallSnapshot(this._payload);
...
class LegacyFavouriteWallsProvider {
  LegacyFavouriteWallsProvider(BuildContext context, {required bool listen})
      : _bloc = listen ? context.watch<FavouriteWallsBloc>() : context.read<FavouriteWallsBloc>();

  final FavouriteWallsBloc _bloc;

  List<LegacyFavouriteWallSnapshot>? get liked {
    final state = _bloc.state;
    if (state.status == LoadStatus.initial) {
      return null;
    }
    return state.items.map((item) => LegacyFavouriteWallSnapshot(item.payload)).toList(growable: false);
  }

  Future<List<LegacyFavouriteWallSnapshot>?> getDataBase() async {
    await _ensureLoaded();
    return liked;
  }
...
extension FavouriteWallsLegacyBridgeX on BuildContext {
  LegacyFavouriteWallsProvider favouriteWallsLegacyProvider({bool listen = true}) {
    return LegacyFavouriteWallsProvider(this, listen: listen);
  }
}
