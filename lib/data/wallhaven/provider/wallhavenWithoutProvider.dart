...
// SHIM: delete in Phase 8
Future<List<WallhavenWallpaper>> categoryDataFetcher(
  String categoryName,
  String mode,
  int? categories,
  int? purity,
  ...
  result.fold(
    onSuccess: (List<WallhavenWallpaper> fetched) {
      if (refresh) {
        ...
        walls.addAll(fetched);
      }
      logger.d("categoryDataFetcher done: ${fetched.length}");
    },
    onFailure: (failure) {
      logger.e("categoryDataFetcher failed: ${failure.message}");
    },
  );
  return walls;
}
...
// SHIM: delete in Phase 8
Future<List<WallhavenWallpaper>> getData(String mode, int? categories, int? purity) async {
  final bool refresh = mode == "r";
  if (refresh) {
    walls = [];
    pageGetData = 1;
  ...
  result.fold(
    onSuccess: (List<WallhavenWallpaper> fetched) {
      ...
      logger.d("getData done: ${fetched.length}");
    },
    onFailure: (failure) {
      logger.e("getData failed: ${failure.message}");
    },
  );
  return walls;
}
